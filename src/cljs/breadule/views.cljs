...
(defn main-panel []
  (let [name (re-frame/subscribe [::subs/name])
        ...
        currentScheduleId (re-frame/subscribe [::subs/currentScheduleId])
    ...
    [:div
                 ...
                 (when (not= @currentScheduleId nil)
                   [md-icon-button
                    ...
                    :on-click (fn []
                                ...
                                (start-wait-timer @stageNum (:stages (get @schedules @currentScheduleId))))])
                 [md-icon-button
                  :size :larger
     ...
     (when (not= @currentScheduleId nil)
       (schedule-view @currentScheduleId (get @schedules @currentScheduleId)))]))
